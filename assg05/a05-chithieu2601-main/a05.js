// 
// a05.js
// Skeleton for CSC444 Assignment 05, Fall 2025
// Joshua A. Levine <josh@arizona.edu>
//
// This file provides the skeleton code for you to write for A05.  It
// generates (using index.html and data.js) grids of 50x50 rectangles 
// to visualize the Hurricane Isabel dataset.
//
// Your main task is to complete the four color functions.
// Additionally, you may want to add additional logic to insert color
// legends for each of the four plots.  These can be inserted as new svg
// elements in the spans colorlegend-X for X=1..4 
//



//////////////////////////////////////////////////////////////////////////////
// Global variables, preliminaries to draw the grid of rectangles

var svgSize = 500;
var bands = 50;

var xScale = d3.scaleLinear().domain([0, bands]).  range([0, svgSize]);
var yScale = d3.scaleLinear().domain([-1,bands-1]).range([svgSize, 0]);

function createSvg(sel)
{
    return sel
        .append("svg")
        .attr("width", svgSize)
        .attr("height", svgSize);
}

function createRects(sel)
{
    return sel
        .append("g")
        .selectAll("rect")
        .data(data)
        .enter()
        .append("rect")
        .attr("x", function(d) { return xScale(d.Col); })
        .attr("y", function(d) { return yScale(d.Row); })
        .attr("width", 10)
        .attr("height", 10)
}

d3.selection.prototype.callAndReturn = function(callable)
{
    return callable(this);
};

//////////////////////////////////////////////////////////////////////////////
// Color functions -- Implement these!

// These are function I implemented

// minimun and maximum value of temperature and pressure
minT = d3.min(data, d => d.T);
maxT = d3.max(data, d => d.T);

minP = d3.min(data, d => d.P);
maxP = d3.max(data, d => d.P);
max = Math.max(Math.abs(minP),Math.abs(maxP))

// color 1 and 2 use the temperature value 
//There are 2 different function for the color but I choose d3.interpolateHcl even I don't really understand the different
function colorT1(d) {
    let color1 = d3.scaleLinear()
            .domain([minT, maxT]) 
            .range(['beige','green']);
    
    return color1(d.T);
}

function colorT2(d) {
    let color2 = d3.scaleLinear()
        .domain([minT, maxT]) 
        .range(['beige','green'])
        .interpolate(d3.interpolateHcl);
    
    return color2(d.T);
}

// This function gives a visualization of the pressure field
// 0 to neutral color
// And the 2 boundaries are opposite so the find the absolute max value
function colorP3(d) {
    let color3 = d3.scaleLinear()
        .domain([-max, 0, max])
        .range(['blue', 'white', 'purple'])
        .interpolate(d3.interpolateHcl);
    
    return color3(d.P); 
}
// But I like this version better since the max looks more detailed for me
function colorP3(d) {
    let color3 = d3.scaleLinear()
        .domain([minP, 0, maxP])
        .range(['blue', 'white', 'purple'])
        .interpolate(d3.interpolateHcl);
    
    return color3(d.P); 
}

// This is the combination of the color3 and color2
// I actually don't know how to find the color, range at first so this is the best I can go with since during multiple fixing, it has the differenciate on the graph
// for c value (middle value), i choose 32 because the example from the instruction choose 32
// I don't know what is the maximum and minimum value hcl can take so I choose as distingused I can
function colorPT4(d) {
    let color1 = d3.scaleLinear()
            .domain([minT, maxT]) 
            .range([50, 90]);
    
    let color3 = d3.scaleLinear()
        .domain([-max, 0, max])
        .range([320, 150, 5]);

    return d3.hcl(color3(d.P), 32, color1(d.T));
}


//////////////////////////////////////////////////////////////////////////////
// Hook up the color functions with the fill attributes for the rects


d3.select("#plot1-temperature")
    .callAndReturn(createSvg)
    .callAndReturn(createRects)
    .attr("fill", colorT1);

d3.select("#plot2-temperature")
    .callAndReturn(createSvg)
    .callAndReturn(createRects)
    .attr("fill", colorT2);

d3.select("#plot3-pressure")
    .callAndReturn(createSvg)
    .callAndReturn(createRects)
    .attr("fill", colorP3);

d3.select("#plot4-bivariate")
    .callAndReturn(createSvg)
    .callAndReturn(createRects)
    .attr("fill", colorPT4);



